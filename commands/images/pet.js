...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("pet")
        .setDescription("Membuat avatar dielus-elus")
        .addUserOption((option) =>
            option
                .setName("user")
                .setDescription("Pengguna yang ingin dielus-elus")
        )
    ...
    async execute(interaction) {
        ...
        try {
            const imageUrl =
                "https://api.popcat.xyz/v2/pet?image=" +
                user.displayAvatarURL({ extension: "png" });

            await interaction.editReply({
                files: [new AttachmentBuilder(imageUrl, { name: "pet.gif" })],
            });
        } catch (error) {
