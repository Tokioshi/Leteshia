...
module.exports = async (client) => {
                        ...
                        try {
                            ...
                            if (!command.data) {
                                console.warn(
                                    ...
                                    chalk.white(
                                        `Command at ${filePath} missing data property`
                                    )
                                );
    ...
    if (commands.length === 0) {
        console.error(
            chalk.red("[ERROR]"),
            chalk.white("No valid commands found to deploy")
        );
        return;
    }
    ...
    if (!client.user?.id) {
        console.error(
            chalk.red("[ERROR]"),
            chalk.white(
                "Client user ID not available. Make sure bot is logged in."
            )
        );
        return;
    }
    ...
    try {
        ...
        if (failedCommands.length > 0) {
            console.log(
                chalk.yellow("[WARNING]"),
                chalk.white(
                    `Note: ${failedCommands.length} commands failed to load`
                )
            );
        }
    } catch (error) {
        console.error(
            chalk.red("[ERROR]"),
            chalk.white("Failed to refresh application commands")
        );
        ...
        if (error.code === 50001) {
            console.error(
                chalk.red("[ERROR]"),
                chalk.white(
                    "Missing Access - Bot may not have necessary permissions"
                )
            );
            ...
            console.error(
                chalk.red("[ERROR]"),
                chalk.white("Rate Limited - Too many requests")
            );
        } else {
            console.error(
                chalk.red("[ERROR]"),
