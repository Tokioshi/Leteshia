...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("gun")
        .setDescription("Dapatkan hamparan Senjata yang sempurna pada avatar!")
        .addUserOption((option) =>
            option
                .setName("user")
                .setDescription("Pilih avatar yang di inginkan!")
                .setRequired(true)
        )
    ...
    async execute(interaction) {
        ...
        try {
            const imageUrl =
                "https://api.popcat.xyz/v2/gun?image=" +
                user.displayAvatarURL({ extension: "png" }) +
                "&text=" +
                encodeURIComponent(text);

            await interaction.editReply({
                files: [new AttachmentBuilder(imageUrl, { name: "gun.png" })],
            });
        } catch (error) {
