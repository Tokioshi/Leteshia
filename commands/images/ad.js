...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("ad")
        .setDescription("Buat avatar menjadi iklan!")
    ...
    async execute(interaction) {
        if (interaction.channel.id !== interaction.client.config.channel.images) {
        ...
        try {
            const imageUrl =
                "https://api.popcat.xyz/v2/ad?image=" +
                user.displayAvatarURL({ extension: "png" });

            await interaction.editReply({
                files: [new AttachmentBuilder(imageUrl, { name: "ad.png" })],
            });
        } catch (error) {
