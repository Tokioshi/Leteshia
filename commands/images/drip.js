...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("drip")
        .setDescription(
            "Berpura-puralah menjadi orang kaya dengan mengenakan jaket mahal palsu!"
        )
    ...
    async execute(interaction) {
        ...
        try {
            const imageUrl =
                "https://api.popcat.xyz/v2/drip?image=" +
                user.displayAvatarURL({ extension: "png" });

            await interaction.editReply({
                files: [new AttachmentBuilder(imageUrl, { name: "drip.png" })],
            });
        } catch (error) {
